...
pars$av_r = 0.1#0.185 # 0.2 range of the niche
...
pars$e_1neg <- 0.01 #0.5 # Scaling factor for out-degree
pars$a_eneg <- 0.1 #0.2  Shape of the exponential increase

pars$beta_ext <- 0.5 # Control strength between out-degree and similarity

pars$competition_coefficient <- 0.05
...
# Timesteps to plot
timesteps_to_plot <- c(1, 5, 10, 20, 30, 40)

 list_network_cropped <- lapply(network_list, function(mat) {
   mat[-c(1:25), ]  # Remove the first 25 rows
 })
...
list_adj_matrix.f <- list(list_network[length(list_network)],
                        list_network[length(list_network)])
