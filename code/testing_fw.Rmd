...
pars$sd <- 0.5 * pars$av_r + 0.001  # Increase the trait variability during speciation
...
For this test, we will subsample the larger matrices to match the size of the smallest matrix in your list (i.e., the number of species at the beginning of your simulation). 
...
normalize the Procrustes correlation values by matrix size to check for scaling effects

```{r}
# Run Procrustes and normalize by matrix size
proc_corr_scaled <- sapply(seq_along(list_int_dist_mean), function(i) {
  int_mat <- list_int_dist_mean[[i]]
  phylo_mat <- list_phylo_dist[[i]]
  proc <- protest(int_mat, phylo_mat)
  proc$t0 / nrow(int_mat) # Normalize by matrix size
})

proc_corr_scaled # Normalized Procrustes correlations
```
