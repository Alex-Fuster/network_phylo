...
sim_model = function(seed, pars, nsteps) {
  ...
  with(pars, {
    ...
    for(step in 2:nsteps) {
      ...
      # Compute the SVD of the interaction matrix
      svd_result <- svd(L[(Sbasal + 1):(Sbasal + Smax), ])
      
      # Extract species positions as predators from the SVD
      V <- svd_result$v  # Positions of species as predators
      
      # Compute cosine similarity between species positions as predators
      similarity_matrix <- V %*% t(V)
      
      # Compute the average similarity for each species as a predator
      avg_similarity <- rowMeans(similarity_matrix)
      
      # Update extinction probability based on the average similarity
